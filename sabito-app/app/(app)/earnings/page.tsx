...
export default function EarningsPage() {
  ...
  if (error) return <div className="workspace-page" role="alert"><p>{error}</p><Button onClick={() => { setLoading(true); setError(""); setAttempt(a => a + 1); }}>Try again</Button></div>;
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-brand-900">Earnings</h1>
          <p className="text-sm text-brand-500">Commissions and cashout requests</p>
        </div>
        <Link href="/cashout">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-brand-200 bg-[#1F4039] p-4 text-white">
        ...
        <div className="rounded-2xl border border-brand-200 bg-white p-4">
          <p className="text-sm text-brand-500">Total earned</p>
          <p className="text-2xl font-bold text-brand-900">GHS {totalEarned.toFixed(2)}</p>
        </div>
      </div>

      <section className="mt-10">
        <h2 className="font-semibold text-brand-900">Commission history</h2>
        <div className="mt-3 space-y-2">
          {earnings.length === 0 ? (
            <p className="text-sm text-brand-500">No earnings yet.</p>
          ) : (
            earnings.map((e) => (
              <div key={String(e.id)} className="rounded-xl border border-brand-200 bg-white p-3 text-sm">
      ...
      <section className="mt-10">
        <h2 className="font-semibold text-brand-900">Cashout requests</h2>
        <div className="mt-3 space-y-2">
          {cashouts.length === 0 ? (
            <p className="text-sm text-brand-500">No cashout requests yet.</p>
          ) : (
            cashouts.map((c) => (
              <div key={String(c.id)} className="rounded-xl border border-brand-200 bg-white p-3 text-sm">
                <p className="font-medium">
