...
export default function ReferralDetailPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <h1 className="mt-4 text-2xl font-bold text-brand-900">Referral details</h1>
      {error ? <p className="mt-4 text-sm text-red-600">{error}</p> : null}
      {!error && !referral ? (
        <p className="mt-4 text-sm text-brand-500">Loading…</p>
      ) : null}
      {referral ? (
        <div className="mt-6 space-y-3 rounded-2xl border border-brand-200 bg-white p-5 text-sm">
          <p>
            <span className="text-brand-500">Client</span>
            <br />
      ...
      {referral && <section className="mt-8" aria-label="Client jobs">
        ...
        <p className="mt-1 text-sm text-brand-500">Progress updates from your partner business. Refreshes every 30 seconds while this page is open.</p>
        {!Array.isArray(referral.jobs) || referral.jobs.length === 0 ? <p className="mt-4 rounded-xl border border-brand-200 bg-white p-5 text-sm text-brand-600">{referral.status === "matched" ? "No linked jobs yet. New jobs will appear once the business starts work for this client." : "Jobs become visible after your referral is matched to a customer."}</p> : <div className="mt-4 space-y-3">{(referral.jobs as AnyRow[]).map(job => <article key={String(job.id)} className="rounded-xl border border-brand-200 bg-white p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs text-brand-500">{String(job.jobNumber)}</p><h3 className="mt-1 font-semibold">{String(job.title)}</h3></div><span className="workspace-status">{String(job.status).replaceAll("_", " ")}</span></div><div className="mt-3 flex flex-wrap gap-4 text-xs text-brand-500">{job.dueDate ? <span>Due {new Date(String(job.dueDate)).toLocaleDateString()}</span> : null}<span>Updated {new Date(String(job.updatedAt)).toLocaleDateString()}</span></div></article>)}</div>}
      </section>}
    </div>
  );
}
