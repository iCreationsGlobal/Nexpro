...
const CashoutRequestScreen: React.FC<CashoutRequestScreenProps> = ({ navigation, route }) => {
  ...
  const [withdrawalFee, setWithdrawalFee] = useState<number>(2); // Default 2%
  ...
  const loadData = async (): Promise<void> => {
    try {
      ...
      if (userData) {
        ...
        // Set withdrawal fee based on subscription plan
        const plan = typeof parsedUser.subscriptionPlan === 'string' 
          ? parsedUser.subscriptionPlan.toLowerCase() 
          : '';
        setWithdrawalFee(plan === 'professional' ? 1 : 2);
      }
  ...
  const fetchEarnings = async (): Promise<void> => {
    try {
      const response = await apiClient.get('/api/marketer/earnings');
      
      if ((response.data as any).earnings) {
        // Filter only available earnings
        const availableEarnings = ((response.data as any).earnings as Earning[]).filter((e: Earning) => 
          e.status === 'paid' && 
        ...
        setSelectedProjects(availableEarnings.map(e => e.project.id));
      }
    } catch (error: any) {
      setEarnings([]);
    }
  ...
  const submitCashoutRequest = async (): Promise<void> => {
    try {
      ...
      await apiClient.post('/api/marketer/cashout-request', payload, {
        headers: {
      ...
      const errorMessage = (error.response?.data as any)?.message || 'Failed to submit cashout request';
      setShowConfirmModal(false);
      showDialog({
        title: 'Error',
