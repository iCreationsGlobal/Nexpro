...
import type { Referral, ApiResponse } from '../../types/api';
...
const MarketerReferralDetailsScreen: React.FC<MarketerReferralDetailsScreenProps> = ({ navigation, route }) => {
  ...
  const fetchReferralDetails = async (backgroundRefresh: boolean = false): Promise<void> => {
    ...
    try {
      const response = await apiClient.get<{ referral: Referral }>(`/api/referrals/${referralId}`);
      if (response.data?.referral) {
        setReferral(response.data.referral);
      } else {
        console.warn('Referral data not found in response:', response.data);
      }
    } catch (error: any) {
