...
import type { User as UserType, Business, PricingPlan } from '../../types/api';

type BusinessAccountProps = RootStackScreenProps<'BusinessAccount'>;

interface SubscriptionData {
  business?: Business;
  user?: UserType;
  planConfig?: PricingPlan | null;
  usage?: any;
}
...
const BusinessAccount: React.FC<BusinessAccountProps> = ({ navigation }) => {
  ...
  const fetchSubscriptionData = async (): Promise<void> => {
    try {
      // Use the same endpoint as web - gets business, user, plan config, and usage in one call
      ...
      if (response.data.success && response.data.data) {
        ...
        setUser(response.data.data.user as UserType);
      }
    } catch (error) {
      ...
      try {
        ...
        if (userData) {
          ...
          if (businessData) {
            ...
            // Create minimal subscription data structure
            setSubscriptionData({
      ...
      buttons: [
        ...
        },
        {
          ...
          onPress: async () => {
            ...
            // Clear theme preference so it defaults to system after logout
  ...
  const handleDeleteAccount = async (): Promise<void> => {
    ...
    try {
      ...
      await AsyncStorage.removeItem('refreshToken');
      await AsyncStorage.removeItem('business');
      // Clear theme preference so it defaults to system after account deletion
      await AsyncStorage.removeItem('theme');
  ...
  const settingsSections: SettingsSection[] = [
    {
      ...
      items: [
        { 
          ...
          onPress: () => {
            navigation.navigate('AllActivities' as any, { userType: 'business' });
          } 
        },
        { 
          icon: BarChart3, 
          ...
          onPress: () => {
            navigation.navigate('BusinessReports' as any);
          } 
        },
      ],
    },
    {
      ...
      items: [
        { 
          icon: UserPlus, 
          label: 'Invites', 
          subtitle: 'Manage team invitations', 
          onPress: () => {
            navigation.navigate('Invites' as any);
          } 
        },
        { 
          icon: Users, 
          label: 'Team Members', 
          subtitle: subscriptionData?.business?.partneredMarketersCount ? `${subscriptionData.business.partneredMarketersCount} members` : 'View and manage your team', 
          onPress: () => {
            navigation.navigate('TeamMembers' as any);
          } 
        },
      ],
    },
    {
      ...
      items: [
        { 
          icon: CreditCard, 
          label: 'Platform Fees', 
          subtitle: 'Fees owed to Sabito', 
          onPress: () => {
            navigation.navigate('PlatformFees' as any);
          } 
        },
        { 
          icon: CreditCard, 
          label: 'Marketer Fees', 
          subtitle: 'Fees owed to marketers', 
          onPress: () => {
            navigation.navigate('MarketerFees' as any);
          } 
        },
        { 
          icon: Crown, 
          label: 'Subscription', 
          subtitle: subscriptionData?.planConfig ? `${subscriptionData.planConfig.name} Plan` : subscriptionData?.business?.subscriptionPlan || 'Manage your plan', 
          onPress: () => {
        ...
        { 
          icon: Percent, 
          label: 'Commissions Setup', 
          subtitle: 'View platform fees & marketer commission rates', 
          onPress: () => {
            navigation.navigate('CommissionsSetup' as any);
          } 
        },
      ],
    },
    {
      ...
      items: [
        { 
          ...
          onPress: () => {
            alert(`DEBUG: Current theme is "${theme}". Check if System option is selected in Theme Settings.`);
  ...
  if (!user) {
    return (
      <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]}>
        <StatusBar barStyle={isDark ? 'light-content' : 'dark-content'} backgroundColor={colors.background} />
        <View style={styles.loadingContainer}>
          <Text style={[styles.loadingText, { color: colors.textSecondary }]}>No account data found</Text>
        </View>
      </SafeAreaView>
    );
  }
      ...
      <ScrollView style={[styles.scrollView, { backgroundColor: colors.background }]} showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          <View style={styles.profileInfo}>
            ...
            <Text style={[styles.profileType, { color: colors.textSecondary }]}>{subscriptionData?.business?.businessName || user.email}</Text>
          </View>
        ...
        <TouchableOpacity 
          ...
          onPress={() => navigation.navigate('Organisation' as any)}
          activeOpacity={0.7}
        >
          <View style={[styles.quickActionIconContainer, { 
            ...
            <Building2 size={22} color="#3B82F6" strokeWidth={1.5} />
          </View>
          <View style={styles.quickActionContent}>
            <Text style={[styles.quickActionText, { color: colors.text }]}>Organisation</Text>
            <Text style={[styles.quickActionDescription, { color: colors.textSecondary }]}>
              {subscriptionData?.business?.businessName || 'Manage business details'}
            </Text>
          </View>
          <ChevronRight size={20} color={colors.iconSecondary} strokeWidth={1.5} />
        </TouchableOpacity>
        ...
        {/* App Version */}
        <Text style={[styles.versionText, { color: colors.textTertiary }]}>Sabito Business v1.0.0</Text>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  profileHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: COLORS.WHITE, // Will be overridden inline
  ...
  // Quick Action Cards (Organisation, Notifications)
  quickActionCard: {
    flexDirection: 'row',
    alignItems: 'center',
