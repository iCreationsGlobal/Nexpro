...
import { X, CreditCard, Calendar, Percent } from 'lucide-react-native';
...
interface PlatformFeePaymentModalProps {
  ...
  fee: Fee | null;
  onClose: () => void;
  onPaymentSuccess?: (data: { amount: number; reference: string }) => void;
}
...
const PlatformFeePaymentModal: React.FC<PlatformFeePaymentModalProps> = ({ visible, fee, onClose, onPaymentSuccess }) => {
  ...
  const handlePaystackSuccess = async (response: PaystackResponse): Promise<void> => {
    try {
      ...
      // Update platform fee payment status on backend
      await apiClient.post(
        '/api/platform-fees/pay',
        {
          invoiceId: fee?.id,
        ...
        {
          timeout: 30000, // 30 second timeout (backend processes payment, creates records)
        }
      );
      // Call success callback FIRST (parent will close modal, show dialog and refresh)
      if (onPaymentSuccess) {
        onPaymentSuccess({ amount: fee?.amount || 0, reference: paystackReference });
      }
      ...
      <View style={styles.modalOverlay}>
        <View style={[styles.modalContent, { backgroundColor: colors.cardBackground }]}>
          ...
          <View style={[styles.modalHeader, { borderBottomColor: colors.border }]}>
            <Text style={[styles.modalTitle, { color: colors.text }]}>Pay Platform Fee</Text>
            <TouchableOpacity
          ...
          <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
            ...
            <View style={[styles.summarySection, { backgroundColor: colors.backgroundSecondary }]}>
              <Text style={[styles.summaryTitle, { color: colors.text }]}>Fee Details</Text>
              
              <View style={styles.summaryRow}>
                <Text style={[styles.summaryLabel, { color: colors.textSecondary }]}>Project:</Text>
                <Text style={[styles.summaryValue, { color: colors.text }]}>
                  {fee.project?.projectName || 'Platform Fee'}
                </Text>
              </View>
              ...
              <View style={styles.summaryRow}>
                <View style={styles.labelWithIcon}>
                  <Calendar size={14} color={colors.iconSecondary} />
                  <Text style={[styles.summaryLabel, { color: colors.textSecondary }]}>Due Date:</Text>
                </View>
                <Text style={[styles.summaryValue, { color: colors.text }]}>
            ...
            <View style={[styles.infoBox, { backgroundColor: isDark ? colors.backgroundSecondary : '#E8F5E9' }]}>
              <Text style={[styles.infoText, { color: colors.textSecondary }]}>
                Your payment will be processed securely via Paystack. You can pay using mobile money, card, or bank transfer.
              </Text>
            </View>
...
export default PlatformFeePaymentModal;
