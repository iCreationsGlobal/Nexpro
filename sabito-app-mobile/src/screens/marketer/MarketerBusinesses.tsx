...
const MarketerBusinesses: React.FC<MarketerBusinessesScreenProps> = ({ navigation }) => {
  ...
  const fetchBusinesses = async (): Promise<void> => {
    try {
      const [{ businesses }, applications, partnerships] = await Promise.all([
        fetchPublicBusinesses({ search: searchQuery.trim() || undefined }),
        listMyApplications().catch(() => []),
        listMyPartnerships().catch(() => []),
      ]);

      const appliedTenantIds = new Set<string>();
      const statusByTenant = new Map<string, 'pending' | 'accepted' | 'rejected'>();

      (applications || []).forEach((app: any) => {
        const tid = String(app.tenantId || app.tenant?.id || '');
        if (!tid) return;
        appliedTenantIds.add(tid);
        if (app.status === 'pending') statusByTenant.set(tid, 'pending');
        else if (app.status === 'declined' || app.status === 'rejected') statusByTenant.set(tid, 'rejected');
        else if (app.status === 'approved') statusByTenant.set(tid, 'accepted');
      });

      (partnerships || []).forEach((p: any) => {
        const tid = String(p.tenantId || p.tenant?.id || '');
        if (!tid) return;
        appliedTenantIds.add(tid);
        statusByTenant.set(tid, p.status === 'active' ? 'accepted' : 'rejected');
      });

      setBusinesses(
        (businesses || []).map((b: any) => {
          const tid = String(b.businessId || b.id);
          return {
            ...b,
            id: b.slug || tid,
            hasApplied: appliedTenantIds.has(tid),
            partnershipStatus: statusByTenant.get(tid),
          } as ExtendedBusiness;
        })
      );
    } catch {
  ...
  const handleCardPress = (business: ExtendedBusiness): void => {
    navigation.navigate('BusinessDetails', {
      businessId: business.slug || business.id,
      ...
      <View style={styles.cardFooter}>
        <View style={[styles.commissionBox, { backgroundColor: isDark ? themeColors.backgroundSecondary : '#F0FDF4' }]}>
          <Text style={[styles.commissionText, { color: themeColors.text }]}>
            Commission: {business.firstClientRatePercent || business.commissionRateNew || 15}% new ·{' '}
            {business.returningClientRatePercent || business.commissionRateReturning || 10}% returning
          </Text>
        </View>
