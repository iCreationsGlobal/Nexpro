...
import { createReferral, listMyPartnerships } from '../../api/absMarketer';
...
const AddReferralScreen: React.FC<AddReferralScreenProps> = ({ navigation }) => {
  ...
  const fetchPartneredBusinesses = async (): Promise<void> => {
    try {
      ...
      const partneredBusinesses: PartnerBusiness[] = (partnerships || []).map((p: any) => ({
        partnershipId: p.id,
        businessName:
          p.tenant?.partnerProgramSettings?.displayName
          || p.tenant?.name
          || 'Business',
        industry: p.tenant?.partnerProgramSettings?.slug,
      }));
      ...
      if (partneredBusinesses.length === 1) {
        setFormData(prev => ({
          ...prev,
          businessName: partneredBusinesses[0].businessName || '',
          partnershipId: partneredBusinesses[0].partnershipId,
        }));
      }
    } catch (error: any) {
      showDialog({
  ...
  const validateForm = (): boolean => {
    ...
    if (!email && !phone) {
      newErrors.clientEmail = 'Provide client email or phone';
      newErrors.clientPhone = 'Provide client email or phone';
    ...
    try {
      if (!formData.partnershipId) {
        showDialog({
          ...
          message: 'Choose the partnered business for this referral.',
          buttons: [{ text: 'OK' }],
        });
        return;
      }
      ...
      const statusNote =
        result?.status === 'matched'
          ? 'Matched to an ABS customer — commissions will accrue when they pay.'
          : result?.status === 'conflict'
            ? 'This customer is already attributed to another marketer.'
            : 'Saved as pending until an ABS customer matches this email or phone.';
      showDialog({
        title: 'Success',
      ...
      <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]}>
        ...
        <View style={styles.emptyContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('MarketerTabs', { screen: 'Businesses' })}
          >
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={[styles.formCard, { backgroundColor: colors.cardBackground, borderColor: colors.border }]}>
          ...
          <View style={styles.inputGroup}>
            <Text style={[styles.label, { color: colors.text }]}>
              Client Email <Text style={[styles.optional, { color: colors.textSecondary }]}>(or phone)</Text>
            </Text>
            <TextInput
          ...
          <View style={styles.inputGroup}>
            <Text style={[styles.label, { color: colors.text }]}>
              Client Phone <Text style={[styles.optional, { color: colors.textSecondary }]}>(or email)</Text>
            </Text>
            <TextInput
          ...
          <View style={styles.inputGroup}>
            <Text style={[styles.label, { color: colors.text }]}>
              Location <Text style={[styles.optional, { color: colors.textSecondary }]}>(optional)</Text>
            </Text>
            <TextInput
          ...
          <View style={[styles.modalContent, { backgroundColor: colors.cardBackground }]}>
            ...
            <ScrollView style={styles.modalScroll}>
              {businesses.map((business) => (
                <TouchableOpacity
                  key={business.partnershipId}
                  style={[
                    styles.businessOption,
                    { borderBottomColor: colors.border },
                    formData.partnershipId === business.partnershipId && { backgroundColor: isDark ? colors.backgroundSecondary : '#E8F5E9' }
                  ]}
                  onPress={() => {
                    setFormData((prev) => ({
                      ...prev,
                      businessName: business.businessName || '',
                      partnershipId: business.partnershipId,
                    }));
