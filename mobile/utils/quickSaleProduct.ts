 ...
 * in code instead of by a person.
 */

const QUICK_SALE_PRODUCT_NAME = 'Quick Sale';

const cacheKey = (tenantId: string) => `quick_sale_product_id_${tenantId}`;

export type QuickSaleProduct = { id: string; name: string };

async function findExisting(tenantId: string): Promise<QuickSaleProduct | null> {
  const cachedId = await AsyncStorage.getItem(cacheKey(tenantId));
  if (cachedId) {
    try {
  ...
  const found = Array.isArray(rows)
    ? rows.find(
        (p: { name?: string }) =>
          (p.name || '').trim().toLowerCase() === QUICK_SALE_PRODUCT_NAME.toLowerCase()
      )
    : null;
  return found?.id ? { id: found.id, name: found.name } : null;
}

export async function getOrCreateQuickSaleProduct(tenantId: string): Promise<QuickSaleProduct> {
  const existing = await findExisting(tenantId);
  if (existing) {
    await AsyncStorage.setItem(cacheKey(tenantId), existing.id);
    return existing;
  }
  ...
  await AsyncStorage.setItem(cacheKey(tenantId), product.id);
  return { id: product.id, name: product.name || QUICK_SALE_PRODUCT_NAME };
}
